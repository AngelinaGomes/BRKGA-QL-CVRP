...
int main()
{
    ...
    while (!feof(arqProblems))
    {
        ...
        ReadData(nameTable, name, region, capacity, vertex, dist);

        // Print a report
        printReport(name, region, capacity, vertex);

        // Print the distance matrix
        printDistMatrix(name, region, vertex, dist);

        // updates n
        n = static_cast<int>(vertex.size());
        n--;
        ...
        for (int j=0; j<MAXRUNS; j++)
        {
            ...
            for (int ib = 0; ib < n; ib++) {
                vec.push_back(bestSolution.vec[ib].sol);
            }
            // printSolution(name, region, capacity, vec, vertex, dist);
            printSolutionLoggi(name, region, capacity, vec, vertex, dist, bestVehicles);
            
            WriteRoutes(bestVehicles, instance, bestSolution.fo);
...
#pragma omp parallel for num_threads(MAX_THREADS)
    for (int i=0; i<p; i++)
    {
        ...
        vector<Vehicle> v{};
        ind = Decoder(ind, v, n, capacity, vertex, dist);
        Pop[i] = PopInter[i] = ind;

        // save the best solution found in this run
        updateBestVehicles(Pop[i], v);
    ...
    while(1)
    {
        ...
        //update epsilon with new value (tentativa de evitar o QL para o epsilon)
        //epsilon = 0.10 + (1-(double)currentTime/MAXTIME) * (0.90 - 0.10);
...
#pragma omp parallel for num_threads(MAX_THREADS)
        ...
        for (int i = (int)(p*pe); i < p - (int)(p*pm); i++)
        {
            ...
            vector<Vehicle> v{};
            PopInter[i] = Decoder(PopInter[i], v, n, capacity, vertex, dist);

            // save the best solution found in this run
            updateBestVehicles(PopInter[i], v);
            updateBestSolution(PopInter[i]);
            // updateBests(PopInter[i], v);
            ...
            if (R >= 1 || stagnation > 10)
            {
                ...
                for (int i=0; i < (int)(p*pe); i++)
                {
                    ...
                    else // pertubar
                    {
                        ...
                        vector<Vehicle> v{};
                        Pop[i] = Decoder(Pop[i], v, n, capacity, vertex, dist);
...
#pragma omp parallel for num_threads(MAX_THREADS)
                for (unsigned int i=0; i < promisingSol.size(); i++)
                {
                    ...
                    TSol s = LocalSearch2(Pop[promisingSol[i]], v, n, dist);

                    updateBestVehicles(s, v);
...
void updateBestVehicles(TSol s, vector<Vehicle> v)
{
    // save the best routes found in this run
    if (s.fo < bestSolution.fo)
    {
        bestVehicles = v;
    }
...
void updateBests(TSol s, vector<Vehicle> v)
{
    // save the best solution found in this run
    if (s.fo < bestSolution.fo)
    {
...
TSol CreateInitialSolutions()
{
    ...
    for (int j = 0; j < n+1; j++)
    {
        ...
        if (j < n)
            aux.sol = j+1; //considerar as requisicoes de 1 a n
        else
            aux.sol = -1;
